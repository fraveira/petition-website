...
app.get("/petition", (req, res) => {
    if (req.session.signatureId) {
        ...
        db.checkSoloSignature(req.session.userId)
            .then(({ rows }) => {
                if (rows[0]) {
                    res.redirect("/thanks");
                } else {
...
app.post("/petition", (req, res) => {
    ...
    db.createSupport(signature, req.session.userId) // add user-id here?
        .then(result => {
...
app.post("/profile/edit", function(req, res) {
    ...
    if (password) {
        bcrypt
            ...
            .then(password => {
                db.editPasswordPlusOthers(
                    ...
                    .then(() => {
                        db.editUsersProfile(age, city, url, req.session.userId)
                            .then(() => {
                                res.redirect("/thanks");
                            })
                            .catch(err => {
        ...
        db.editUsersInfo(first, last, email, req.session.userId)
            .then(() => {
                return db
...
app.listen(process.env.PORT || 8080, () =>
    console.log("Petition Server running succesfully")
);
